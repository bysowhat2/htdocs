...
//绝对路径了
require_once dirname(__FILE__).'/../libraries/WxJSAPIPay.class.php';
...
class Waimai extends CI_Controller {
	...
	public function getCode($type){
		$scope = 'snsapi_base';
		$appid = 'wx8cc15c84c45b1c5e';
		//$scope = 'snsapi_base';
		//必须与公众平台上设置的获取信息接口domain相同
		$redirect_uri = 'http://www.yuebuting.com/index.php/waimai/getOpenid';
		
		$data = array();
		//函数名
		$data['type'] = $type;
		//获取订座参数
		$data['date'] = isset($_GET['date'])?$_GET['date']:FALSE;
		$data['hour'] = isset($_GET['hour'])?$_GET['hour']:FALSE;
		$data['min'] = isset($_GET['min'])?$_GET['min']:FALSE;
		$data['number'] = isset($_GET['number'])?$_GET['number']:FALSE;
		$data['name'] = isset($_GET['name'])?$_GET['name']:FALSE;
		$data['telephone'] = isset($_GET['telephone'])?$_GET['telephone']:FALSE;
		$data['sex'] = isset($_GET['sex'])?$_GET['sex']:FALSE;
		$data['others'] = isset($_GET['others'])?$_GET['others']:FALSE;
		$data = json_encode($data);
		
		//$type参数在state中传递，公众号的要求-_-!
		$url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.$redirect_uri.'&response_type=code&scope='.$scope.'&state='.$data.'#wechat_redirect';
		header("Location: $url");
	}
	...
	public function getOpenid(){
		...
		$state = isset($_GET['state'])?$_GET['state']:FALSE;//调用哪个函数如付费(pay),订座（seatonly)等信息+订座信息
		$data = json_decode($state, true);
		$type = $data['type'];
		$data['code'] = $code;
		
		if($code === FALSE){//无code处理
			$this->logError('code is FALSE in function getOpenid');
			$this->load->view('pageError.php');
			return;
		}
 		$jsAPIPay = new WxJSAPIPay($code);
		$openid = $jsAPIPay->get_user_openid();//获取openid
		
		if($type === FALSE || !method_exists($this, $type)){//无type处理
			$this->logError('type is FALSE in function getOpenid');
			$this->load->view('pageError.php');
			return;
		} else {
			//按照函数名，调用不同函数
			$this->$type($openid, $data);
		}
	}
	
	public function seatOnly($openid = FALSE, $data = FALSE){
		if($openid === FALSE || $data === FALSE){
			$this->logError('openid OR data is FALSE in function seatOnly');
			$this->load->view('pageError.php');
			return;
		}
		//加载订座成功的页面
		$this->load->view('successOrder', $data);//显示订座成功的页面。
		
		
		
		
/* 		//通过公众号发送订座信息给用户(客服接口）
		//如果用户和公众号48小时内有互动，例如发送过消息，点击过菜单，那么可以48小时内可以主动发送客服消息。
		$fromUsername = 'wx8cc15c84c45b1c5e';
		$toUsername = $openid;
		$contentStr = "<br/>姓名： $data[name]";
		if($data['sex'] == 1)
			$contentStr .= "先生<br/>";
		else 
			$contentStr .= "女士<br/>";
		$contentStr .= "日期：$data[date]<br/>时间： $data[hour]点$data[min]分<br/>人数： $data[number]<br/>电话：$data[telephone]<br/>";
		$time = time();
		$msgType = "text";
		$textTpl = "<xml>
